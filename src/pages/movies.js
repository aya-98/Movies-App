...
function Movies(){
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
           const  arr = res.data.results.slice(0,14)
            setmovies(arr)
    ...
    <div className="row">
        {
            ...
            movies.map((movie , ind)=>{
                if(! arr4.includes(movie.id) ){
                   ...
                   <div className="card  " >
                       <Link to= {`/showmovie/${movie.id}`} ><img className="card-img-top" style={{height:"250px"}} src={"https://image.tmdb.org/t/p/w500/" +movie.poster_path}/> </Link>
